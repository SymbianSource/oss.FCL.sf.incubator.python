...
class OtherFileTests(unittest.TestCase):
    ...
    def testUnicodeOpen(self):
        ...
        if sys.platform != 'symbian_s60':
            ...
            # As TESTFN contains '\' it complicates things when the file is 
            # opened in unicode as the two slashes are retained
            # repr(f) adds two more slashes and comparing it to TESTFN fails
            self.assert_(repr(f).startswith("<open file u'"))
        f.close()
        os.unlink(TESTFN)
