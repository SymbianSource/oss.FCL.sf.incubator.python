...
class MixinStrUnicodeUserStringTest:
    ...
    def test_floatformatting(self):
        ...
        for prec in xrange(100):
            format = '%%.%if' % prec
            value = 0.01
            # On the Symbian platform a value greater than 1e49 limits the 
            # precision to 28 above which it crashes the interpreter with 
            # panic User24. Hence reducing the value to 1e49 from 1e59 so 
            # that the test passes ( i.e. reduced range of x from 60 to 50 )
            # Added platform specific check in the native modules 
            # stringobject.c and unicodeobject.c so that an OverflowError is 
            # raised for values above 1e49 and precision greater than 28.
            if sys.platform == 'symbian_s60':
                xvalue = 50
            else:
                xvalue = 60
            for x in xrange(xvalue):
