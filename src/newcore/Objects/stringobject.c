...
/* Used Py_CHARMASK because char is 'signed' by default on Symbian and */
/* all string replace functions call memchr with second argument as char. */
/* When characters were beyond the range(>127) memchr would return NULL. */
/* Added a Symbian check as windows and BSD both have signed char as */
/* default but string replace still works on these platforms     */
#ifdef SYMBIAN
#define findchar(target, target_len, c)				\
	((char *)memchr((const void *)(target), Py_CHARMASK(c), target_len))
#else
#define findchar(target, target_len, c)				\
            ...
            int prec, int type, PyObject *v)
{
	...
	/* Worst case length calc to ensure no buffer overrun:
	   ...
	   On the Symbian platform for a value greater than 1e49 the 
       precision is limited to 28 above which it crashes the interpreter
       with panic User24.  Added a check so that OverflowError is raised
       for values greater than 1e49 with precision greater than 28.
	*/
	
	if (
	    #ifdef SYMBIAN
	    fabs(x) >= 1e50 && (size_t)prec > 28 ||
	    #endif
	    ((type == 'g' || type == 'G') &&
