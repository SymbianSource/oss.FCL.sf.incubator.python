...
open_the_file(PyFileObject *f, char *name, char *mode)
{
	...
	if (f->f_fp == NULL) {
		...
		if (errno == 0)	/* bad mode string */
			...
			errno = ENOENT;
#endif
/* A Call to fopen/open in OpenC on Symbian, returns an errno 22
(i.e EINVAL) if a file path contains a multiple character drive 
name (something like "not_c:\\file"), instead of returning 
errno 2 (i.e ENOENT). Hence we choose to ignore this condition 
and just return the filename instead
*/
#ifndef SYMBIAN
		if (errno == EINVAL)
