...
new_arena(void)
{
	...
	if (unused_arena_objects == NULL) {
		...
		arenaobj = (struct arena_object *)PyCore_REALLOC_FUNC(arenas, nbytes);
		if (arenaobj == NULL)
			return NULL;
		arenas = arenaobj;
	...
	arenaobj->address = (uptr)PyCore_MALLOC_FUNC(ARENA_SIZE);
...
redirect:
	...
	if (nbytes == 0)
		nbytes = 1;
	return (void *)PyCore_MALLOC_FUNC(nbytes);
}
		...
		*(block **)p = lastfree = pool->freeblock;
		...
		if (lastfree) {
			...
			if (nf == ao->ntotalpools) {
				...
				PyCore_FREE_FUNC((void *)ao->address);				
...
PyObject_Realloc(void *p, size_t nbytes)
{
	...
	if (nbytes)
	{
		return PyCore_REALLOC_FUNC(p, nbytes);
	}
