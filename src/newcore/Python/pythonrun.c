...
/* On Symbian/OpenC, we need to explictly set text-mode as the default stdio*/
#ifdef SYMBIAN
	set_fmode('t');
#endif
	...
	if (install_sigs) {
		/* Disable signal handling on Symbian as the sigaction API 
		 * in OpenC 1.5 crashes. */
#ifndef SYMBIAN
...
Py_Finalize(void)
{
    ...
    /* Destroy data related to the special thread functionalities provided */
    PyThread_finalize_thread();
#endif
...
/* Adding implementation for Symbian */
#ifdef SYMBIAN
extern int symbian_check_stack(void);
DL_EXPORT(int)
PyOS_CheckStack(void)
{
	return symbian_check_stack();
}
#endif /* SYMBIAN */
