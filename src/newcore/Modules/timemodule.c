...
#ifdef SYMBIAN
extern double symbian_clock(void);
#endif
...
{
#ifdef SYMBIAN
	return PyFloat_FromDouble(symbian_clock());
#else
 ...
 time_mktime(PyObject *self, PyObject *tup)
{
	...
	buf = *localtime(&tt);
	TEST_PROBE;
	//printf("Before: %d %d %d %ld\n", buf.tm_year, buf.tm_mon, buf.tm_mday, buf.tm_gmtoff);
	if (!gettmarg(tup, &buf))
		return NULL;
    //printf("After: %d %d %d %ld\n", buf.tm_year, buf.tm_mon, buf.tm_mday, buf.tm_gmtoff);
	TEST_PROBE;
	tt = mktime(&buf);
	TEST_PROBE;
	if (tt == (time_t)(-1)) {
	...
	TEST_PROBE;
	ret = PyFloat_FromDouble((double)tt);
	TEST_PROBE;
	return ret;
}
